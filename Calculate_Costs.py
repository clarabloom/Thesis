...
def calculate_cost(df, period, base_dir):
    ...
    if ("case" in base_dir.lower()) | (period == 1):
        ...
        try:
            caprev_nj.append(round(((resmar['CapRes_8'].values*captarget) * hourly_weights * hourly_demand).sum()/(hourly_weights * hourly_demand).sum()))
        except KeyError:
            caprev_nj.append(0)
...
def main():
    ...
    out_path = os.path.join(script_dir, "Costs_by_Zone_by_Year_ALL_CASES.csv")
